...
// "human") — the canvas measures #constellation-col-{key}'s full bounding box
// (label + one-liner) at runtime, so each cluster sits below the ENTIRE text
// block, never just the label row (see SignalField's computeClusterLayout).
const ENTRIES = [
  ...
  return (
    <section id="constellation" className="border-y border-signal/10">
      <div className="mx-auto grid w-full max-w-6xl gap-10 px-6 py-20 sm:grid-cols-3 sm:gap-6">
        {ENTRIES.map((entry, i) => (
          <Reveal key={entry.label} delay={i * 0.1}>
            <div id={`constellation-col-${entry.key}`} className="flex flex-col gap-2.5">
              <p className="flex items-center gap-3 font-mono text-xs uppercase tracking-[0.3em] text-ink">
